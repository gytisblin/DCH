function [layer,layer2,layer3,layer4,layer5]= pfisr_hypothesis_input_1(year,month,day,starttime,endtime,start_hour,start_min,end_hour,end_min,doyin,hour,hour2,hour3,min1,min2,min3,PRN)
...
timestamp=[];
if ~isempty(Madrigal)
timestamp=unique(Madrigal(:,16))
end
timestampac=[];
if ~isempty(Madrigalac)
timestampac=unique(Madrigalac(:,16))
end
Maxlp=[];
Maxac=[];
save_maxNe_lp=[];
...
[layer2,layer3,sizeranges_ac,total_data_points_ac,rangesnotnansum_total_ac,nesum_total_ac,rdata_scint_ac,gdac]=check_data_ac_19(year,month,day,start_hour,start_min,end_hour,end_min,doyin,hour,hour2,hour3,min1,min2,min3,PRN,Madrigal,Madrigalac)
if isempty(gdac)
    gdac=0
end
...
[layer4,layer5,sizeranges_lp,total_data_points_lp,rangesnotnansum_total_lp,nesum_total_lp,rdata_scint_lp,gdlp]=check_data_lp_19(year,month,day,start_hour,start_min,end_hour,end_min,doyin,hour,hour2,hour3,min1,min2,min3,PRN,Madrigal,Madrigalac)
...
for i=1:length(scinttime)
	rows=find(Madrigal(:,1)==-154.3 & ...
		Madrigal(:,12)==77.5 & ...
		Madrigal(:,16)==scinttime(i) & ...
		Madrigal(:,4)>=195);




madrigal_rows_data=Madrigal(rows,7);
madrigal_rows_error=Madrigal(rows,19);
...
maxrow=find(Madrigal(rows,7)==Maxlp);
...
if isfinite(Maxlp)
    if Maxlp==-999
        ...
        if isempty(save_maxNe_lp)==1
        save_maxNe_lp=[Madrigal(rows(maxrow),16),Madrigal(rows(maxrow),4),Madrigal(rows(maxrow),7),Madrigal(rows(maxrow),19)];
        else
        save_maxNe_lp=[save_maxNe_lp;[Madrigal(rows(maxrow),16),Madrigal(rows(maxrow),4),Madrigal(rows(maxrow),7),Madrigal(rows(maxrow),19)]];
        end
...
rowsac=find(Madrigalac(:,1)==-154.3&Madrigalac(:,12)==77.5&Madrigalac(:,16)==scinttimeac(i)&Madrigalac(:,4)<195)

madrigal_rows_data_ac=Madrigalac(rowsac,7)
madrigal_rows_error_ac=Madrigalac(rowsac,19)
...
maxrowac=find(Madrigalac(rowsac,7)==Maxac);
...
if isfinite(Maxac)
    if Maxac==-999
        ...
        if isempty(save_maxNe_ac)==1
        save_maxNe_ac=[Madrigalac(rowsac(maxrowac),16),Madrigalac(rowsac(maxrowac),4),Madrigalac(rowsac(maxrowac),7),Madrigalac(rowsac(maxrowac),19)];
        else
        save_maxNe_ac=[save_maxNe_ac;[Madrigalac(rowsac(maxrowac),16),Madrigalac(rowsac(maxrowac),4),Madrigalac(rowsac(maxrowac),7),Madrigalac(rowsac(maxrowac),19)]];
        end
